...
_DISABLE_TORCH_COMPILE = (
    os.getenv("ENABLE_TENSORRT", "False").lower() == "true"
    or os.getenv("DISABLE_TORCH_COMPILE", "False").lower() in ("1", "true", "yes")
    or os.getenv("TORCHDYNAMO_DISABLE", "0") == "1"
)
if not _DISABLE_TORCH_COMPILE and torch.cuda.is_available():
    _cap = torch.cuda.get_device_capability(0)
    _DISABLE_TORCH_COMPILE = _cap >= (12, 1)
...
class FlowUniPCMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    @torch.compile(mode="reduce-overhead", fullgraph=True, dynamic=False, disable=_DISABLE_TORCH_COMPILE)
