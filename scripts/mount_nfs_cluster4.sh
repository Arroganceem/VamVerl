...
#
# 41 首次: sudo bash scripts/setup_nfs_exports_41.sh
#
# 用法（worker 上 robotem 通常已 sudo 免密，无需 SUDO_PASS）:
#   bash scripts/mount_nfs_cluster4.sh [all|models|vamverl|data|clips|fstab|status]
#   PERSIST_FSTAB=1 bash scripts/mount_nfs_cluster4.sh vamverl
#   WORKER_IPS="21" bash scripts/mount_nfs_cluster4.sh all
set -euo pipefail
...
_mount_one() {
  ...
  if ! _sudo mount -t nfs -o "${NFS_OPTS}" "${src}" "${dst}"; then
    echo "  FAIL ${name}: NFS 挂载失败（41 上是否已 sudo bash scripts/setup_nfs_exports_41.sh ?）" >&2
    return 1
  fi
  if [[ ! -e "${probe}" ]]; then
...
cmd_mount() {
  ...
  if [[ "${_fail}" -ne 0 ]]; then
    ...
    echo "    sudo bash ${ROOT}/scripts/setup_nfs_exports_41.sh" >&2
