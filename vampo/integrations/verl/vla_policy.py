...
from vampo.imagination.policy.obs_utils import convert_rl_obs_to_vla_obs
...
class VLAPolicyModule(nn.Module):
    ...
    def _log_prob_from_trace(self, trace: FlowJointTrace) -> torch.Tensor:
        """Exact log π from stored path/ε (trace-parse fallback)."""
    ...
    def sample_step(
        ...
        """Rollout sampling only; log π is filled later via ``actor.compute_log_prob`` (verl hybrid)."""
        ...
        placeholder_lp = torch.zeros((), dtype=torch.float32, device=self.device)
        return action, video, placeholder_lp, trace
    ...
    def _chain_log_prob(
        ...
        *,
        ...
        """verl-style actor forward for log π (flow trace replay), with trace-parse fallback."""
        try:
            ...
            if torch.isfinite(lp).all() and float(lp.abs()) > 0:
                return lp
        except Exception as exc:
            if enable_grad:
                raise
            print(f"VAMPO WARNING: log_prob forward failed ({exc}); trace fallback", flush=True)
        return self._log_prob_from_trace(trace)
    ...
    def _log_prob_for_one_trajectory(
        ...
        *,
        ...
        row_steps = []
        saved_buffer = list(self._video_buffer)
        self.reset_episode()
        ...
        for t in range(traj_len):
            ...
            row_steps.append(lp)
        self._video_buffer = saved_buffer
