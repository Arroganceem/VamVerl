...
def _local_wan_dir(repo_id: str = WAN_HF_REPO_ID) -> str | None:
    if repo_id == WAN_HF_REPO_ID:
        return os.environ.get("WAN21_DIR", "/home/robotem/Models/Wan2.1-I2V-14B-480P")
    if repo_id == WAN22_HF_REPO_ID:
        return os.environ.get("WAN22_DIR", "/home/robotem/Models/Wan2.2-TI2V-5B")
    return None


def _flow_rl_gp_log_prob(target: torch.Tensor, mean: torch.Tensor, sigma: float) -> torch.Tensor:
    var = sigma * sigma
    return (-0.5 * (((target.float() - mean.float()) ** 2) / var + math.log(2 * math.pi * var))).sum()


def _flow_rl_standard_normal_log_prob(sample: torch.Tensor) -> torch.Tensor:
    return (-0.5 * (sample.float() ** 2 + math.log(2 * math.pi))).sum()


def _flow_rl_recompute_log_prob_from_trace(
    trace_action_path: list[torch.Tensor],
    trace_action_eps: list[torch.Tensor],
    trace_video_path: list[torch.Tensor],
    trace_video_eps: list[torch.Tensor],
    *,
    action_sigma: float,
    video_sigma: float,
    device: torch.device,
) -> torch.Tensor:
    """CPU float64 log π from stored path/ε (Megatron-safe; avoids GPU acc NaN)."""
    total = torch.zeros((), dtype=torch.float64)
    ap0 = trace_action_path[0].detach().float().cpu()
    vp0 = trace_video_path[0].detach().float().cpu()
    total = total + _flow_rl_standard_normal_log_prob(ap0).double()
    total = total + _flow_rl_standard_normal_log_prob(vp0).double()
    for i in range(len(trace_action_eps)):
        a_target = trace_action_path[i + 1].detach().float().cpu()
        a_eps = trace_action_eps[i].detach().float().cpu()
        a_mean = a_target - float(action_sigma) * a_eps
        total = total + _flow_rl_gp_log_prob(a_target, a_mean, action_sigma).double()

        v_target = trace_video_path[i + 1].detach().float().cpu()
        v_eps = trace_video_eps[i].detach().float().cpu()
        v_mean = v_target - float(video_sigma) * v_eps
        total = total + _flow_rl_gp_log_prob(v_target, v_mean, video_sigma).double()
...
def ensure_file(path: str | None, hf_filename: str, repo_id: str = WAN_HF_REPO_ID) -> str:
    """Return a valid local path: config path, WAN*_DIR, then HuggingFace."""
    if path is not None and os.path.exists(path):
        return path
    local_dir = _local_wan_dir(repo_id)
    if local_dir:
        local_path = os.path.join(local_dir, hf_filename)
        if os.path.exists(local_path):
            return local_path
    return hf_download(hf_filename, repo_id)


def _load_dit_safetensors_into_model(model: nn.Module, dit_dir: str, *, device: str = "cpu") -> None:
    """Load DiT weights shard-by-shard to avoid merging a full ~62GB state_dict."""
    ...
    if os.path.exists(safetensors_index_path):
        ...
        for shard_file in sorted(set(index["weight_map"].values())):
            ...
            shard_state_dict = load_file(shard_path, device=device)
...
class WANPolicyHead(ActionHead):
    ...
    def __init__(
        ...
        self.flow_rl_sigma = float(os.getenv("VAMPO_FLOW_RL_SIGMA", "0.05"))
        self.flow_rl_video_sigma = float(os.getenv("VAMPO_FLOW_RL_VIDEO_SIGMA", str(self.flow_rl_sigma)))
        ...
        if should_load_checkpoint_weights():
            ...
            self.image_encoder.model.load_state_dict(
                torch.load(img_enc_path, map_location=load_dev, weights_only=True), strict=False
            )
        ...
        if not config.skip_component_loading and should_load_checkpoint_weights():
            ...
            if dit_dir is None or not os.path.isdir(dit_dir):
                local_dir = _local_wan_dir(dit_repo_id)
    ...
    def set_trainable_parameters(self, tune_projector: bool, tune_diffusion_model: bool):
        ...
        if tune_projector and not tune_diffusion_model:
            for head_part in ("action_decoder", "action_encoder", "state_encoder"):
                mod = getattr(self.model, head_part, None)
                if mod is not None:
    ...
    def inject_lora_after_loading(self):
        ...
        if self.train_architecture != "lora":
            ...
            return
        if any("lora" in name.lower() for name, _ in self.model.named_parameters()):
            print("LoRA adapters already injected; skipping duplicate injection")
            return
    ...
    def _create_kv_caches(
        ...
        head_dim = getattr(self.model, "head_dim", self.model.dim // num_heads)
    ...
    def lazy_joint_video_action(
        ...
        *,
        ...
        rl_action_path: torch.Tensor | None = None,
        ...
        trace_video_path: list[torch.Tensor] = []
        trace_video_eps: list[torch.Tensor] = []
        flow_log_prob_total: torch.Tensor | None = None
        rl_sigma = self.flow_rl_sigma
        rl_video_sigma = self.flow_rl_video_sigma

        if rl_mode == "log_prob":
            if rl_action_path is None or rl_action_eps is None:
                raise ValueError("rl_log_prob requires rl_action_path and rl_action_eps")
            if rl_video_path is None or rl_video_eps is None:
                raise ValueError("rl_log_prob requires rl_video_path and rl_video_eps")
            noise_action = rl_action_path[0].to(device=image.device, dtype=torch.bfloat16)
        ...
        elif rl_mode == "trace" and rl_action_path is not None:
            noise_action = rl_action_path[0].to(device=image.device, dtype=torch.bfloat16)
        else:
        ...
        if rl_mode == "trace":
            trace_action_path.append(noise_action.detach().float().cpu())
            flow_log_prob_total = torch.zeros((), device=image.device, dtype=torch.float32)
            flow_log_prob_total = flow_log_prob_total + _flow_rl_standard_normal_log_prob(noise_action)
        ...
        for index, current_timestep in enumerate(sample_scheduler.timesteps):
            ...
            mu_video = sample_scheduler.step(
            ...
            mu_action = sample_scheduler_action.step(
            ...
            if rl_mode == "log_prob":
                vtarget = rl_video_path[index + 1].to(device=mu_video.device, dtype=torch.float32)
                flow_log_prob_total = flow_log_prob_total + _flow_rl_gp_log_prob(
                    vtarget, mu_video, rl_video_sigma
                )
                veps = rl_video_eps[index].to(device=mu_video.device, dtype=mu_video.dtype)
                noisy_input = mu_video + rl_video_sigma * veps

                atarget = rl_action_path[index + 1].to(device=mu_action.device, dtype=torch.float32)
                flow_log_prob_total = flow_log_prob_total + _flow_rl_gp_log_prob(
                    atarget, mu_action, rl_sigma
                )
                aeps = rl_action_eps[index].to(device=mu_action.device, dtype=mu_action.dtype)
                noisy_input_action = mu_action + rl_sigma * aeps
            elif rl_mode == "trace":
                veps = torch.randn_like(mu_video)
                noisy_input = mu_video + rl_video_sigma * veps
                trace_video_path.append(noisy_input.detach().float().cpu())
                trace_video_eps.append(veps.detach().float().cpu())
                flow_log_prob_total = flow_log_prob_total + _flow_rl_gp_log_prob(
                    noisy_input, mu_video, rl_video_sigma
                )

                aeps = torch.randn_like(mu_action)
                noisy_input_action = mu_action + rl_sigma * aeps
                trace_action_path.append(noisy_input_action.detach().float().cpu())
                trace_action_eps.append(aeps.detach().float().cpu())
                flow_log_prob_total = flow_log_prob_total + _flow_rl_gp_log_prob(
                    noisy_input_action, mu_action, rl_sigma
                )
            else:
                noisy_input = mu_video
                noisy_input_action = mu_action
        ...
        if rl_mode == "trace" and trace_action_path:
            flow_log_prob_total = _flow_rl_recompute_log_prob_from_trace(
                trace_action_path,
                trace_action_eps,
                trace_video_path,
                trace_video_eps,
                action_sigma=rl_sigma,
                video_sigma=rl_video_sigma,
                device=image.device,
            )
        ...
        return BatchFeature(
            data={
                ...
                **(
                    {
                        "action_flow_path": torch.stack(trace_action_path, dim=0),
                        "action_flow_eps": torch.stack(trace_action_eps, dim=0),
                        "video_flow_path": torch.stack(trace_video_path, dim=0),
                ...
                **(
                    ...
                    if rl_mode == "log_prob" and flow_log_prob_total is not None
                    else {}
                ),
            }
        )
    ...
    def post_initialize(self):
        ...
        skip_compile = (
            ENABLE_TENSORRT
            or os.getenv("DISABLE_TORCH_COMPILE", "False").lower() in ("1", "true", "yes")
            or os.getenv("TORCHDYNAMO_DISABLE", "0") == "1"
            or getattr(torch._dynamo.config, "disable", False)
        )
        if not skip_compile and torch.cuda.is_available():
            skip_compile = torch.cuda.get_device_capability(0) >= (12, 1)
