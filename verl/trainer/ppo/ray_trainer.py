...
class RayTrainer(object):
    ...
    def _create_dataloader(self):   # next fix
        ...
        from verl.utils.dataset.rob_dataset import StateDataset
    ...
    def _validate(self, global_steps=0):
        ...
        for test_data in self.val_dataloader:
            ...
            test_batch.meta_info = {
                ...
                'validate': True,
                # 'save_to_hdfs': True, # TODO
                # 'return_rollouts': True, # TODO
                # 'rollout_base_dir': self.config.actor_rollout_ref.rollout_base_dir, #TODO
                "global_steps":global_steps,
            }
            test_output_gen_batch = self.actor_rollout_wg.generate_sequences(test_batch)
            print("test_output_gen_batch", test_output_gen_batch)
    ...
    def _maybe_epoch_cooldown(self, global_steps: int) -> None:
        ...
        every = int(
            os.environ.get(
                ...
                self.config.trainer.get("cooldown_every_steps", 1) or 0,
            )
            or 0
        )
        sec = float(
            os.environ.get(
                "VAMPO_COOLDOWN_SEC",
                self.config.trainer.get("cooldown_sec", 0) or 0,
            )
            or 0
        )
        if every <= 0 or sec <= 0 or global_steps <= 0 or global_steps % every != 0:
            return
        print(f"cooldown @ global_step {global_steps}: sleep {sec:.0f}s")
        time.sleep(sec)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            if True:
                ...
                global_steps += 1
                self._maybe_epoch_cooldown(global_steps)
