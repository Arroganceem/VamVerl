...
# 用法: sudo bash scripts/cluster/setup_nfs_exports_41.sh
set -euo pipefail
...
if [[ "$(id -u)" -ne 0 ]]; then
  ...
  echo "    sudo bash scripts/cluster/setup_nfs_exports_41.sh" >&2
  exit 1
fi
...
echo ">>> worker: bash scripts/cluster/mount_nfs_cluster4.sh all"
