...
from verl.workers.rollout.imagination.policy.obs_utils import (
    append_imagined_frame,
    build_obs_with_video_history,
    init_frame_buffers,
)
from verl.workers.rollout.imagination.policy.runner import PolicyRunner
from verl.workers.rollout.imagination.reward_debug import debug_reward_enabled, dump_trajectory_video
    ...
    def get(self, index: int) -> tuple[str, dict, str]:
        if not self.entries:
            raise RuntimeError(
                ...
                "run: bash scripts/data/build_init_states_from_droid.sh"
            )
...
class ImaginationRollout:
    ...
    def rollout_one(
        ...
        from verl.utils.dreamzero_parallel import seed_rollout_sample
        ...
        from verl.utils.reward.wmpo_sliding import frame_finish_to_wm_step
