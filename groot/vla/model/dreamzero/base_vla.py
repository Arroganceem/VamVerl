    ...
    def lazy_joint_video_action_causal(
        ...
        **rl_kwargs,
        ...
        action_head_outputs = self.action_head.lazy_joint_video_action(
            backbone_outputs, action_inputs, latent_video=latent_video, **rl_kwargs
        )
        self.validate_data(action_head_outputs, backbone_outputs, is_training=False)
        return action_head_outputs
    ...
    def from_pretrained(
        ...
        if use_fsdp_meta_init():
            ...
            try:
                ...
                print(f"Rank {rank}: meta→empty CPU init OK")
            except Exception as exc:
                print(f"Rank {rank}: meta init failed ({exc}), falling back to to_empty after cls(config)")
                model = cls(config)
                model.to_empty(device="cpu")
        else:
            model = cls(config)
        ...
        def _load_shard_dict(shard_state_dict: dict) -> None:
            if any(".base_layer." in key for key in shard_state_dict.keys()):
                shard_state_dict = {
                    k.replace(".base_layer.", "."): v for k, v in shard_state_dict.items()
                }
        ...
        if should_load_checkpoint_weights():
            ...
            if load_dev == "cuda":
                model = model.to("cuda")
                print("rank0: model on CUDA before safetensor shard load (skip CPU staging)")
            if os.path.exists(safetensors_index_path):
                print(f"Loading sharded safetensors using index: {safetensors_index_path}")
                with open(safetensors_index_path, "r") as f:
                    index = json.load(f)
                for shard_file in sorted(set(index["weight_map"].values())):
                    shard_path = os.path.join(pretrained_model_name_or_path, shard_file)
                    print(f"Loading shard: {shard_path}")
                    shard_state_dict = load_file(shard_path, device=load_dev)
                    _load_shard_dict(shard_state_dict)
                    del shard_state_dict
                    gc.collect()
                    if load_dev == "cuda":
