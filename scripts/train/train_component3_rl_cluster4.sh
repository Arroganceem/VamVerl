...
#
# 用法:
#   bash scripts/train/train_component3_rl_cluster4.sh              # 开训
#   bash scripts/train/train_component3_rl_cluster4.sh stop         # 停四机训练进程（不关 Ray）
#   bash scripts/train/train_component3_rl_cluster4.sh status
set -euo pipefail
...
_TRAIN_PATTERNS=(
  ...
  'DreamZeroActorRolloutRefWorker'
  'ray::DreamZeroActorRolloutRefWorker'
  'ray::main_task'
)
_TRAIN_PGREP='main_dreamzero_ppo|DreamZeroActorRolloutRefWorker|ray::DreamZeroActorRolloutRefWorker|ray::main_task'
...
markers = ("DreamZeroActorRolloutRefWorker", "main_task")
killed = 0
actors = []
try:
...
cmd_launch() {
  ...
  export VAMVERL_RANK0_NODE_IP="${VAMVERL_RANK0_NODE_IP:-192.168.88.31}"
  export WANDB_API_KEY="${WANDB_API_KEY:-}"
  export WANDB_PROJECT="${WANDB_PROJECT:-vamverl}"
  export WANDB_MODE="${WANDB_MODE:-online}"
  export VAMVERL_NCCL_TIMEOUT_MIN="${VAMVERL_NCCL_TIMEOUT_MIN:-120}"
  export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"
  # .41 head 兼 Ray GCS/driver，缩小 object store 给 WorkerDict 留统一内存
  export RAY_OBJECT_STORE_BYTES="${RAY_OBJECT_STORE_BYTES:-4294967296}"
  cd "$ROOT"
...
PY
  ...
  if [[ "${PREFLIGHT_SKIP_REWARD:-0}" == "1" ]]; then
    bash "${ROOT}/scripts/preflight/preflight_rl.sh" --skip-reward
  else
    bash "${ROOT}/scripts/preflight/preflight_rl.sh" --strict-reward
  fi

  # shellcheck source=/dev/null
  source "$ROOT/scripts/data/bootstrap_init_states.sh"
  ensure_init_states
...
PY
  ...
  exec "${ENV_PYTHON}" -m verl.trainer.main_dreamzero_ppo \
