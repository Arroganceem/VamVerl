...
try:
    ...
    # tianshou>=2.0 removed tianshou.policy; inference wrapper only needs nn.Module.
    import torch.nn as _nn

    BaseTianshouPolicy = _nn.Module
import torch
...
def _unfreeze_projector_modules(action_head) -> None:
    """Projector MLPs live on action_head.model, not action_head top-level."""
    for head_part in ("action_decoder", "action_encoder", "state_encoder"):
        for parent in (action_head, getattr(action_head, "model", None)):
            if parent is None:
                continue
            mod = getattr(parent, head_part, None)
            if mod is not None:
...
def _has_lora_parameters(module) -> bool:
    return any("lora" in name.lower() for name, _ in module.named_parameters())


def _apply_rl_lora_finetune(
    action_head,
    *,
    tune_projector: bool = True,
    tune_diffusion_model: bool = True,
) -> None:
    """Inject LoRA adapters on dense checkpoints and enable RL trainable params."""
    ah = action_head
    if ah.train_architecture != "lora":
        print(
            "RL LoRA mode: overriding checkpoint "
            f"train_architecture={ah.train_architecture!r} -> 'lora'"
        )
        ah.train_architecture = "lora"

    if not _has_lora_parameters(ah):
        if not hasattr(ah, "inject_lora_after_loading"):
            raise RuntimeError("action_head does not support LoRA injection")
        ah.inject_lora_after_loading()
    else:
        print("LoRA adapters already present; keeping for RL")

    for enc in ("text_encoder", "image_encoder", "vae"):
        mod = getattr(ah, enc, None)
        if mod is not None:
            mod.requires_grad_(False)
    if hasattr(ah, "model"):
        ah.model.requires_grad_(False)
    ...
    if tune_projector:
        _unfreeze_projector_modules(ah)

    if hasattr(ah, "print_trainable_params"):
        ah.print_trainable_params()

    trainable = sum(p.numel() for p in ah.parameters() if p.requires_grad)
    print(f"RL LoRA trainable parameter count (action_head): {trainable:,}")
    if trainable == 0:
        print("Warning: RL LoRA mode enabled but no trainable parameters found.")


def _apply_rl_finetune_gradients(
    action_head,
    *,
    tune_projector: bool = True,
    tune_diffusion_model: bool = True,
) -> None:
    """Unfreeze action-head weights for RL full fine-tune (VAE/text/image stay frozen)."""
    for enc in ("text_encoder", "image_encoder", "vae"):
        mod = getattr(action_head, enc, None)
        if mod is not None:
            mod.requires_grad_(False)
    if tune_projector:
        _unfreeze_projector_modules(action_head)
    if tune_diffusion_model and hasattr(action_head, "model"):
        for param in action_head.model.parameters():
            param.requires_grad = True
    if hasattr(action_head, "print_trainable_params"):
        action_head.print_trainable_params()
...
class GrootSimPolicy(BaseGrootSimPolicy):
    def __init__(
        ...
        keep_lora_trainable: bool = False,
        rl_fine_tune_mode: str = "lora",
        tune_projector: bool = True,
        tune_diffusion_model: bool = True,
    ):
        """
        ...
        rl_mode = str(rl_fine_tune_mode).lower()
        if keep_lora_trainable and rl_mode != "full":
            rl_mode = "lora"

        if rl_mode == "full":
            print("RL fine-tune mode: full (merge LoRA if present, train dense action head)")
            ah = model.action_head
            if ah.train_architecture == "lora":
                if getattr(ah, "defer_lora_injection", False) and hasattr(ah, "inject_lora_after_loading"):
                    ah.inject_lora_after_loading()
                if hasattr(ah.model, "merge_and_unload"):
                    ah.model = ah.model.merge_and_unload()
            _apply_rl_finetune_gradients(
                ah,
                tune_projector=tune_projector,
                tune_diffusion_model=tune_diffusion_model,
            )
        elif rl_mode == "lora":
            print("RL fine-tune mode: lora (inject/keep LoRA adapters + projector)")
            _apply_rl_lora_finetune(
                model.action_head,
                tune_projector=tune_projector,
                tune_diffusion_model=tune_diffusion_model,
            )
        elif model.action_head.train_architecture == "lora":
            print("Merging LoRA weights into main model weights (frozen)")
            model.action_head.model = model.action_head.model.merge_and_unload()
        else:
            print(
                "No RL fine-tune applied "
                f"(rl_mode={rl_mode!r}, train_architecture={model.action_head.train_architecture!r})"
            )
        ...
        # Store model on CPU for lazy load, or CUDA for rank0 FSDP (skip CPU staging).
        if lazy_load:
            from groot.vla.utils.distributed_load import fsdp_rank0_load_device

            model.to(device=fsdp_rank0_load_device())
        else:
            model.to(device=device)

        # Post initialize, move RoPE freqs to cuda.
        if not defer_post_initialize:
    ...
    def lazy_joint_forward_causal(self, batch, video=None, latent_video=None, state=None, video_only=False, enable_grad: bool = False, **rl_kwargs):
        ...
        with grad_ctx:
            model_pred = self.trained_model.lazy_joint_video_action_causal(
                normalized_input, latent_video=latent_video, **rl_kwargs
            )
        normalized_action = model_pred["action_pred"].float()
        video_pred = model_pred["video_pred"]
        flow_aux = {
        ...
        return batch, video_pred, flow_aux
