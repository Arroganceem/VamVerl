...
from vampo.reward.videomae_dataset import (
    PrecomputedClipDataset,
    collate_clips,
    resolve_shard_globs,
)
from vampo.reward.videomae_load import (
    configure_trainable,
    count_trainable,
    load_videomae_classifier,
    optimizer_param_groups,
)
from vampo.reward.videomae_reward import DEFAULT_VIDEOMAE_BACKBONE
from vampo.reward.videomae_train_metrics import (
...
def _build_loaders(cfg: dict[str, Any]) -> tuple[DataLoader, DataLoader, dict[str, Any]]:
    ...
    if not clip_manifest.is_file():
        raise RuntimeError(
            f"Missing clip dataset at {clip_dir}. Run: bash scripts/prep_component2_data_local.sh"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=os.environ.get(
            ...
            str(Path(__file__).resolve().parents[2] / "configs/videomae_droid.yaml"),
        ),
    )
