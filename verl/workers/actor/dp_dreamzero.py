...
from verl.utils.dreamzero_fsdp_utils import clip_grad_norm_vla
from verl.utils.vla.dreamzero_policy import DreamZeroPolicyModule
from verl import DataProto
from verl.workers.actor import BasePPOActor


class DreamZeroPPOActor(BasePPOActor):
    ...
    def __init__(self, config, actor_module: nn.Module, actor_optimizer: torch.optim.Optimizer | None = None):
        ...
        if not isinstance(actor_module, DreamZeroPolicyModule):
            raise TypeError(f"DreamZeroPPOActor expects DreamZeroPolicyModule, got {type(actor_module)}")
    ...
    def update_policy(
        ...
        data: DataProto,
        *,
        after_micro_backward: Callable[[], None] | None = None,
