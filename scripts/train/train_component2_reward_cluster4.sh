...
#
# 前置: bash scripts/cluster/mount_nfs_cluster4.sh clips
#
# 用法:
#   bash scripts/train/train_component2_reward_cluster4.sh          # 开训
#   bash scripts/train/train_component2_reward_cluster4.sh stop   # 四机停训
#   bash scripts/train/train_component2_reward_cluster4.sh status
set -euo pipefail
...
NGPU=1
NODE_RANK=0
CONFIG="${CONFIG:-$ROOT/verl/utils/reward/configs/videomae_droid.yaml}"
...
set +e
PATTERNS=(
  'verl.utils.reward.train_videomae'
  'torch.distributed.run.*train_videomae'
)
...
_count_on_host() {
  local where="$1"
  local cmd="pgrep -fc 'verl.utils.reward.train_videomae|torch.distributed.run.*train_videomae' 2>/dev/null || echo 0"
  local raw
  if [[ "${where}" == "local" ]]; then
...
exec python -m torch.distributed.run \\
  ...
  -m verl.utils.reward.train_videomae --config '${CONFIG}'
EOF
}
...
cmd_launch() {
  ...
  if ! bash "${ROOT}/scripts/preflight/preflight_component2_train.sh"; then
    exit 1
  fi
