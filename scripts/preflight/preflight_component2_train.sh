...
from verl.utils.reward.train_videomae import _build_loaders, _load_config, _class_weights
...
for ip in 31 21 11; do
  if ssh -o BatchMode=yes -o ConnectTimeout=5 -o "BindAddress=${SRC_BIND:-192.168.88.41}" \
    ...
    _warn "worker 192.168.88.${ip} 无 ${ROOT} → bash scripts/cluster/mount_nfs_cluster4.sh clips"
  fi
  if ssh -o BatchMode=yes -o ConnectTimeout=5 -o "BindAddress=${SRC_BIND:-192.168.88.41}" \
    "${REMOTE_USER:-robotem}@192.168.88.${ip}" "test -f ${CLIP_DIR}/manifest.json" 2>/dev/null; then
    _ok "worker 192.168.88.${ip} clip manifest 可读"
  else
    _warn "worker 192.168.88.${ip} 读不到 clip → bash scripts/cluster/mount_nfs_cluster4.sh clips"
  fi
done
...
if [[ "${fail}" -eq 0 ]]; then
  echo ">>> 检查通过，可以开训:"
  echo "    bash scripts/train/train_component2_reward_cluster4.sh"
else
