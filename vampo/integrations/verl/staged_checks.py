...
from vampo.integrations.verl.preflight import (
    CheckResult,
    _fail,
    _ok,
    _warn,
...
from vampo.integrations.verl.grpo_advantage import compute_vampo_grpo_from_batch
...
VAMPO_IMPORT_MODULES = (
    "vampo.reward.base",
    "vampo.reward.factory",
    "vampo.reward.wmpo_sliding",
    "vampo.reward.videomae_reward",
    "vampo.integrations.verl.log_prob_utils",
    "vampo.integrations.verl.proto_adapter",
    "vampo.integrations.verl.grpo_advantage",
    "vampo.integrations.verl.reward_manager",
    "vampo.integrations.verl.preflight",
    "vampo.integrations.verl.main_vampo_ppo",
    "vampo.imagination.rollout",
    "vampo.data.droid_episode_split",
)
...
def check_static_compile(root: Path) -> CheckResult:
    ...
    if failed:
        return _fail(
            ...
            f"python -m compileall {root / 'vampo'}",
        )
...
def check_rl_pipeline_mock() -> CheckResult:
    ...
    try:
        ...
        rm = VAMPORewardManager(num_examine=0, config=config)
        reward_dict, _metrics = rm(proto)
        proto.batch["token_level_rewards"] = reward_dict["all"]

        adv, _ret = compute_vampo_grpo_from_batch(proto, config)
...
def check_hydra_entry(root: Path) -> CheckResult:
    try:
        from vampo.integrations.verl.main_vampo_ppo import main_hydra

        if not callable(main_hydra):
            return _fail("hydra_entry", "main_hydra 不可调用")
        return _ok("hydra_entry", "main_vampo_ppo.main_hydra 可导入")
    except Exception as exc:
        return _fail(
            "hydra_entry",
            f"训练入口导入失败: {exc}",
...
def run_stage(
    ...
    *,
    ...
    if stage == 4:
        from vampo.integrations.verl.preflight import _load_reward_settings
...
def print_staged_report(
    ...
    *,
    ...
    if train_ready:
        ...
        print("      开训前补跑: bash scripts/verify_staged.sh --through 5")
    else:
