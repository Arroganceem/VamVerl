...
def reorder_pgs_rank0_first(pgs: list[PlacementGroup], rank0_ip: str) -> list[PlacementGroup]:
    ...
    if not head:
        ...
        raise RuntimeError(
            ...
            f"PG node IPs: {ips}. Check Ray cluster / VAMPO_RANK0_NODE_IP."
        )
    ordered = head + others
    print(
        f"[VAMPO] rank0 pinned to {rank0_ip}: PG order → {[pg_node_ip(p) for p in ordered]}",
        flush=True,
    )
    return ordered


class VAMPORayWorkerGroup(RayWorkerGroup):
    """RayWorkerGroup that assigns torch dist rank 0 to VAMPO_RANK0_NODE_IP (default .31)."""
