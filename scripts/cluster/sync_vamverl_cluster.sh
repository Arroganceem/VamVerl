...
#
# 用法:
#   bash scripts/cluster/sync_vamverl_cluster.sh              # 同步 + 校验（默认不 install）
#   bash scripts/cluster/sync_vamverl_cluster.sh --install      # 同步后在各节点 pip install -e
#   bash scripts/cluster/sync_vamverl_cluster.sh status         # 查看各节点 NFS/校验状态
#   WORKER_IPS="21" bash scripts/cluster/sync_vamverl_cluster.sh
set -euo pipefail
...
VERIFY_PY="import numpy, numba, albumentations, tianshou, ftfy, verl; assert numpy.__version__ < '2.5', numpy.__version__; from verl.utils.dreamzero_parallel import get_actor_strategy; print('verl dreamzero ok', 'numpy', numpy.__version__, 'strategy', get_actor_strategy(__import__('omegaconf').DictConfig({'actor': {'strategy': 'fsdp'}})))"
...
cmd_sync() {
  ...
  echo ">>> 同步策略: 代码 rsync；data/ 不传输（先 bash scripts/cluster/mount_nfs_cluster4.sh vamverl）"
  if [[ "${DO_INSTALL}" == "1" ]]; then
