...
def check_episode_split(split_dir: Path) -> CheckResult:
    ...
    try:
        from verl.utils.data.droid_episode_split import load_episode_split
        ...
        return _fail("episode_split", str(exc), "bash scripts/data/build_init_states_from_droid.sh")


def check_init_states(init_dir: Path) -> CheckResult:
    try:
        from verl.utils.data.init_states_bootstrap import validate_init_states
...
def check_init_overlap(init_dir: Path, split_dir: Path) -> CheckResult:
    ...
    try:
        from verl.utils.data.check_episode_overlap import check_init_manifest_overlap
    ...
    if skip_reward:
        results.append(
            _warn(
                ...
                "训练前请运行: bash scripts/preflight/preflight_rl.sh --strict-reward",
            )
        )
        return results
...
def print_report(results: list[CheckResult], *, strict_reward: bool) -> None:
    ...
    print()
    if train_ok:
        print("结论: 可以启动 RL 训练 → bash scripts/train/train_component3_rl_cluster4.sh")
    elif prep_ok and not strict_reward:
        print("结论: 本地数据/环境就绪；reward 服务未做 strict 检查")
        print("      就绪后运行: bash scripts/preflight/preflight_rl.sh --strict-reward")
        print("      然后启动:   bash scripts/train/train_component3_rl_cluster4.sh")
    else:
