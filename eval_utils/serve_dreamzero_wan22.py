...
Usage (single GPU):
  ...
  torchrun --nproc_per_node=1 eval_utils/serve_dreamzero_wan22.py --model_path /path/to/dreamzero_wan22_ckpt --port 8000

  # Or single process:
  python eval_utils/serve_dreamzero_wan22.py --model_path /path/to/dreamzero_wan22_ckpt --port 8000
...
def _maybe_disable_torch_compile() -> None:
    """Skip torch.compile when Triton/ptxas cannot target this GPU (e.g. GB10 sm_121a)."""
    if os.getenv("ENABLE_TORCH_COMPILE", "").lower() in ("1", "true", "yes"):
        return
    explicit = os.getenv("DISABLE_TORCH_COMPILE", "").lower() in ("1", "true", "yes")
    tensorrt = os.getenv("ENABLE_TENSORRT", "").lower() == "true"
    dynamo_off = os.getenv("TORCHDYNAMO_DISABLE", "0") == "1"
    auto = False
    if torch.cuda.is_available():
        major, minor = torch.cuda.get_device_capability(0)
        # Spark GB10 reports capability 12.1; bundled ptxas lacks sm_121a.
        auto = (major, minor) >= (12, 1)
    if explicit or tensorrt or dynamo_off or auto:
        torch._dynamo.config.disable = True
        reason = "env" if (explicit or tensorrt or dynamo_off) else f"sm_{major}{minor}"
        print(f"torch.compile disabled for serve ({reason})")
...
class DreamZeroWan225BPolicy(BasePolicy):
    ...
    def infer(self, obs: dict) -> np.ndarray:
        ...
        with torch.no_grad():
            result_batch, video_pred, *_ = self._policy.lazy_joint_forward_causal(batch)
...
def main(
    model_path: str = "/path/to/dreamzero_wan22_ckpt",
    embodiment_tag: str = "oxe_droid",
