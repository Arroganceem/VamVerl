...
from vampo.imagination.policy.obs_utils import flatten_obs
from vampo.integrations.verl.log_prob_utils import build_rollout_log_prob_tensors
...
def trajectories_to_dataproto(
    ...
    reuse_trace_log_prob: bool = False,
    action_sigma: float | None = None,
    video_sigma: float | None = None,
) -> DataProto:
    """Build verl-compatible TensorDict from imagined trajectories."""
    if not trajectories:
    ...
    if reuse_trace_log_prob:
        ...
        if built is not None:
            ...
            batch["rollout_log_probs"] = per_step
            batch["rollout_log_prob_scalar"] = scalar
